...
/**
 * Demonstrates how to create a custom data structure, and the dangers of returning references of
 * this data structure. As a result, this object is not encapsulated properly, and is not hiding its
 * internal data properly (even though it is set to private).
 */
public class PrefixMap {

  /**
   * Stores strings by the prefix they start with. The key is the prefix String and the value is a
   * sorted set of strings that start with that prefix.
   */
  private final TreeMap<String, TreeSet<String>> internal;

  /**
   * Determines the size of the prefix used. Must be at least one letter. If a string is smaller
   * than the prefix size, it is skipped. Since the value cannot be changed after initialization, it
   * can be safely made public.
   */
  public final int prefixSize;

  /**
   * Initializes the prefix map using the specified size. If an invalid size is provided, will
   * initialize a prefix map of size 1.
   *
  ...
  public PrefixMap(int prefixSize) {
    ...
    /*
     * Remember, this initializes the OUTER data structure only. You need a specific key to pair
     * with the TreeSet before you can initialize the inner data structure.
     */
  ...
  public boolean addWord(String word) {
    ...
    /*
     * if (internal.containsKey(prefix)) { TreeSet<String> words = internal.get(prefix);
     * words.add(word); internal.put(prefix, words); } else { TreeSet<String> words = new
     * TreeSet<>(); words.add(word); internal.put(prefix, words); }
     */

    // VERSION 2: FEWER IDENTIFIERS
    /*
     * if (internal.containsKey(prefix)) { internal.get(prefix).add(word); } else {
     * internal.put(prefix, new TreeSet<>()); internal.get(prefix).add(word); }
     */

    // VERSION 3: NO REPEATED CODE
    /*
     * if (!internal.containsKey(prefix)) { internal.put(prefix, new TreeSet<String>()); }
     *
  ...
  /**
   * Returns a safe copy of the prefixes in the map. The copy is not updated if this prefix map
   * changes.
   *
  ...
  /**
   * Returns a safe copy of the words sharing a prefix. The copy is not updated if this prefix map
   * changes.
   *
   * @param prefix the prefix to get
  ...
  /**
   * Returns an unmodifiable view of the internal map, but the internal data structures are still
   * modifiable.
   *
