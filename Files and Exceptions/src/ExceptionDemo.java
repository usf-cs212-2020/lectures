...
public class ExceptionDemo {
  ...
  /**
   * A simple method to calculate the percentage given the total points earned versus the total
   * points possible. Note that this method does not explicitly throw any exceptions, but an
   * {@link ArithmeticException} will be thrown if the value of possible is 0.
   *
  ...
  /**
   * A simple helper method for outputting the results in a nice format. Not the focus of this
   * example.
   *
   * @param earned points earned
   * @param possible points possible
  ...
  /**
   * Shows what the code looks like if we assume no exceptions or invalid data will occur. It is
   * more straightforward to understand what this code is trying to do, and hence easier to debug.
   *
   * Note: Avoid suppressing warnings in code. It is done here to demonstrate bad practice.
   */
  ...
  /**
   * Handles what can go wrong without catching any exceptions. Instead, we try to constantly
   * validate the input data. This clutters the code quite a bit, making it harder to follow. Also,
   * notice we have a problem if we return too soon without closing the Scanner object.
   */
  @SuppressWarnings("resource")
  ...
  /**
   * This is much easier to read. The try block is as uncluttered as the uncaught demo, and always
   * makes sure the Scanner is closed. However, we are not catching cases with negative values and
  ...
  /**
   * Here, we make several improvements. A try-with-resources block for the Scanner removes
   * the necessity of the finally block. We added back in a little bit of validation where
   * necessary, and made the catch blocks more specific to give better output to the user.
   */
