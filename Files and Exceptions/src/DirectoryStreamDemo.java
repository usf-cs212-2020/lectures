...
/**
 * This class demonstrates how to use a {@link DirectoryStream} to create a recursive file listing.
 *
...
public class DirectoryStreamDemo {
  ...
  /**
   * Traverses through the directory and its subdirectories, outputting all paths to the console.
   * For files, also includes the file size in bytes.
   *
  ...
  private static void traverseDirectory(Path directory) throws IOException {
    /*
     * The try-with-resources block makes sure we close the directory stream when done, to make sure
     * there aren't any issues later when accessing this directory.
     *
     * Note, however, we are still not catching any exceptions. This type of try block does not have
     * to be accompanied with a catch block. (You should, however, do something about the
     * exception.)
     */
