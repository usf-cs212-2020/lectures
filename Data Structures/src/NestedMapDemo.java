...
public class NestedMapDemo {
  ...
  /**
   * Stores a set of words by the length of that word. For example, the word "ant" has 3 characters.
   * The word "bat" also has length 3. So both words should appear in the set for words of length 3.
   *
   * @param args unused
   */
  public static void main(String[] args) {
    ...
    /*
     * For each length word found (i.e. 3 letter word, 4 letter word), will store a set of all words
     * found with that length.
     *
     * Note that this only initializes the outer data structure! We cannot initialize the inner sets
     * until we have keys to pair those sets with.
     */
    ...
    for (String word : text.toLowerCase().split(" ")) {
      ...
      /*
       * This can be confusing. We are getting the set stored WITHIN the map, and adding a word
       * directly. We do not need to call put() again. This is related to mutability and references
       * in Java which we will get into more detail later.
       */
    }

    /*
     * To iterate through a nested data structure, we need one loop per level of nesting. Here we
     * have 2 levels, so we need 2 loops.
     */
