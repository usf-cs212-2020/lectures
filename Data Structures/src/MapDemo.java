...
/**
 * Demonstrates basic map operations using a {@link HashMap} and a {@link TreeMap}.
 */
public class MapDemo {

  /**
   * Test case for this demo. This is another form of the sentence "Buffalo buffalo buffalo buffalo
   * buffalo." See Wikipedia for more.
   *
  ...
  private static void demoMap(Map<String, Integer> map, String text) {
    ...
    for (String word : text.split(" ")) {
      ...
      /*
       * Need to determine if this is the first time we saw the word and need to initialize its
       * value, or if we need to add 1 to the old value already stored.
       */
      ...
      if (map.containsKey(word) == false) {
        ...
        /*
         * This word is already stored, so get the old count from the map and add 1 to the value.
         * Update the results. You can break this up into multiple lines if it is easier to
         * understand!
         */
      ...
      /*
       * Or, we can take advantage of the convenience methods in the map class and get rid of the
       * if/else block entirely.
       */

      // int current = map.getOrDefault(word, 0);
      // map.put(word, current + 1);

      // There is even a one-line option, but it uses lambda functions (discussed later).
      // map.merge(word, 1, Integer::sum);
    }

    /*
     * Print out the map to the console. You can take the easy approach, or iterate through the map
     * via the key set.
     */
    ...
    // We can use var from Java 10+ and an entrySet instead:
