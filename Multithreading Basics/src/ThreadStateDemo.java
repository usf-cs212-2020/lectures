...
/**
 * Demonstrates simple changes in thread state for a main thread and a single worker thread.
 */
public class ThreadStateDemo {
  ...
  /**
   * Gets the current main thread, spawns a new worker thread, and outputs the changes in thread
   * state.
   *
  ...
  /**
   * Worker thread that performs some meaningless computation, that should take longer than the main
   * thread.
   */
  ...
  /**
   * Performs some meaningless computation. Used to keep threads busy without calling sleep() or
   * wait() explicitly.
   *
