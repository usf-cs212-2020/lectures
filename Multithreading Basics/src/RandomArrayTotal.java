...
/**
 * Demonstrates basic multithreading, and illustrates how to break up a problem into subproblems.
 * Also used to motivate the inefficiency of constantly creating new threads instead of reusing
 * them.
 */
public class RandomArrayTotal {

  /**
   * Calculates a subtotal in an array. Does no validation of parameters, so results will vary when
   ...
   * @return subtotal of numbers from index {@code start} (inclusive) to {@code start + chunk}
   *         (exclusive)
   */
  ...
  /**
   * Calculates the total of an array using multithreading. Used to demonstrate the cost of
   * creating/destroying thread objects versus using a work queue.
   *
   * @param numbers array of numbers to total
  ...
  /**
   * Uses the {@link RandomArrayTotal#subtotal(int[], int, int)} method to generate a
   * subtotal of an array.
   */
  ...
  /**
   * Compares our single threaded and multi threaded approach. Notice that we do not see a speedup
   * when we increase the number of threads for small sizes. Do you understand why?
   *
   * @param args unused
  ...
  private static long benchmark(int size, int threads) throws InterruptedException {
    ...
    System.out.printf("%.05f seconds average for %d numbers and %d threads%n", average, size,
        threads);
