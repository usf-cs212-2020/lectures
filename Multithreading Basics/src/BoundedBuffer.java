 ...
 * {@link Thread#wait()} and {@link Thread#notifyAll()} to create a thread-safe data structure. Uses
 * the produce-consumer model to demo this data structure with multiple threads.
 *
...
public class BoundedBuffer<E> {
  ...
  /**
   * Initializes a bounded buffer capable of storing {@code bufferSize} elements at once.
   *
  ...
  /**
   * Places an element into the buffer, or if full, waits until space is available.
   *
  ...
  /**
   * Convenience method for adding multiple items at once. Not the most efficient implementation,
   * but demonstrates that synchronized methods can call other synchronized methods since the thread
  ...
  /**
   * Removes and returns an element from the buffer. If the buffer is empty, waits until there is an
   * element to retrieve.
   *
