...
public class SynchronizedSet<E> extends IndexedSet<E> {
  ...
  /**
   * Initializes a sorted or unsorted synchronized index set depending on the parameter.
   *
  ...
  /*
   * Because this class uses synchronized methods instead of a private object for synchronization,
   * it may be unsafe if the instance of this class is public (since other code will have access to
   * the lock object and can cause a deadlock to occur). If properly used as a private member
   * without breaking encapsulation, it should be fine.
   */
}
