...
/**
 * Demonstrates the time difference between using the synchronized keyword and a read/write lock for
 * an IndexedSet object.
 *
 * @see IndexedSet
 * @see SynchronizedSet
 * @see ConcurrentSet
...
public class SetDemo {
  ...
  /**
   * Attempts to benchmark the differences between synchronization approaches. Try modifying the
   * order operations are performed in the warmup and timed loops. It should (theoretically) not
   * impact which approach is faster.
   *
  ...
  /**
   * Roughly demonstrates runtime different between using synchronized and a read/write lock when
   * there are more than one large read operations. Note: not an accurate benchmark, but you get the
   * idea.
   *
   * @param args unused
   * @throws InterruptedException if interrupted
