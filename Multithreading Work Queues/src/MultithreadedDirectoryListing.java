 ...
 * for making a multithreaded version of {@link RecursiveDirectoryListing}. However, there are still
 * performance issues caused by creating so many little worker thread objects, which have to be
...
public class MultithreadedDirectoryListing {
  ...
  /**
   * Instead of a static worker class, we will create a task manager that can keep track of pending
   * work.
   */
  private static class TaskManager {
    ...
    /**
     * The non-static worker class that will update the shared paths and pending members in our task
     * manager instance.
     */
    private class Worker extends Thread {
      ...
      public void run() {
        /*
         * To avoid a synchronized block within a loop, we will create data that is "local" to this
         * thread (i.e. not shared).
         */
        ...
        try (DirectoryStream<Path> stream = Files.newDirectoryStream(path)) {
          for (Path current : stream) {
            ...
            if (Files.isDirectory(current)) {
              ...
              /*
               * Note that we no longer need to join() on the worker. That means other threads can
               * keep on going without having to wait for each other. Only main needs to wait for
          ...
          /*
           * Now, we will block and make our big update. It is less likely threads will block
           * because they will finish their work at different times, and we spend less time
           * locking/unlocking and hence less time causing blocking
           */
          synchronized (paths) {
            paths.addAll(local);
          }
    ...
    /*
     * Since multiple threads need to modify the pending variable, these methods should be
     * synchronized!
     */

    /**
     * Rather than having threads wait for each other (undoing our multithreading), we will wait
     * until all pending work is completed.
     *
    ...
    /**
     * Safely decrements the shared pending variable, and wakes up any threads waiting for work to
     * be completed.
     */
    private synchronized void decrementPending() {
      ...
      /*
       * How often should we call notifyAll()? We don't want to overdo it. (See log if we take out
       * the if pending == 0.)
       */
